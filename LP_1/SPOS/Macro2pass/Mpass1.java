...
public class Mpass1 {
        public static void main(String[] args) throws IOException {
            ...
            while((line=br1.readLine())!=null){
                ...
                for(int i=0;i<tokens.length;i++){
                    ...
                    if(tokens[i].equalsIgnoreCase("macro")){
                        ...
                        if(macro_start){
                            MNT[mnt_cnt++]=new mnt(tokens[i],mdt_cnt,arglist_cnt);
                            macro_start=false;
                            fill_arglist=true;
                        }
                        if(fill_arglist){
                            int current_macro_arg_count = 0;
                            while(i<tokens.length){
                                ...
                                if(tokens[i].matches("&[a-zA-Z]+")||tokens[i].matches("&[a-zA-Z]+[0-9]+")){
                                    ...
                                    current_macro_arg_count++;
                                }
                                i++;
                            }
                            MNT[mnt_cnt-1].arg_cnt = current_macro_arg_count;
                            fill_arglist=false;
                        }else{
            ...
            for(int i = 0;i<mnt_cnt;i++){
                ...
                bw1.write(MNT[i].name + "\t" + MNT[i].addr + "\t" + MNT[i].arg_cnt + "\n");
            }
            bw1.close();
